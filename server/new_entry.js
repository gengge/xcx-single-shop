//[root@bj74lz server]# cat entry.js

// 引入https 以及 koa-ssl
const https = require('https')
const sslify = require('koa-sslify').default//注意，需要安装这个模块npm install koa-sslify
...
const options = {
 key: fs.readFileSync('自己的路径.key'),
 cert: fs.readFileSync('自己的路径.pem'),
}
...
https.createServer(options, app.callback()).listen(7003, (err) => {
  if (err) {
    console.log('服务启动出错', err);
  } else {
    console.log('guessWord-server运行在' + 7003 + '端口');
  }
});	
